...
def export_events_cli(
    out: Optional[str] = typer.Option(None, "--out", help="Optional output directory"),
    database_url: Optional[str] = typer.Option(None, "--database-url", help="Override DATABASE_URL for this command."),
):
    export_path = Path(out) if out else None
    results = export_events(database_url=database_url, output_dir=export_path)
